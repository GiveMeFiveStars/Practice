...
INSERT INTO `administrator` VALUES ('admin','123456','陈嘉睿',17358651327,'轩然公司'),('fine','789','姜宇涵',17358639907,'轩然公司'),('good','123','张雨轩',17300224091,'轩然公司'),('well','7488','刘雨欣',17546981200,'轩然公司');
...
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `cargo` (
  `g_id` varchar(255) NOT NULL COMMENT '货物快递号',
...
CREATE TABLE `machine` (
  ...
  KEY `machine_ibfk_1` (`stash_id`) USING BTREE,
  CONSTRAINT `machine_ibfk_1` FOREIGN KEY (`stash_id`) REFERENCES `stash` (`stash_id`) ON DELETE SET NULL ON UPDATE CASCADE
...
CREATE TABLE `stash` (
  ...
  `s_time` date DEFAULT NULL COMMENT '租赁时间',
  `c_time` date DEFAULT NULL COMMENT '截止时间',
...
CREATE TABLE `vehicle` (
  ...
  `stash_id` int DEFAULT NULL COMMENT '所属仓库',
  `e_id` int DEFAULT NULL COMMENT '驾驶员',
  `license_id` int NOT NULL COMMENT '许可证编号',
  `t_limit` date DEFAULT NULL COMMENT '有效期限',
  PRIMARY KEY (`v_id`) USING BTREE,
  KEY `vehicle_ibfk_2` (`e_id`) USING BTREE,
  KEY `vehicle_ibfk_3` (`stash_id`) USING BTREE,
  CONSTRAINT `vehicle_ibfk_2` FOREIGN KEY (`e_id`) REFERENCES `employee` (`e_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `vehicle_ibfk_3` FOREIGN KEY (`stash_id`) REFERENCES `stash` (`stash_id`) ON DELETE SET NULL ON UPDATE CASCADE
...
-- Dump completed on 2023-07-20 16:00:29
